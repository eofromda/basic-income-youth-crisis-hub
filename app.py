...
st.set_page_config(
    page_title="Basic Income × Youth Crisis Hub",
    page_icon="📊",
    layout="wide",
...
st.markdown(
    ...
    .main-title {
        ...
        background: linear-gradient(90deg, #4F8CFF, #FFB020, #32D583, #A78BFA);
    ...
    .question-box {
        background: linear-gradient(135deg, rgba(79,140,255,0.18), rgba(167,139,250,0.14));
...
TEXT = {
    "ko": {
        ...
        "source_note": "출처: KOSIS, OECD Data, Bank of Korea, 공공 정책 보고서 기반 교육용 재구성",
        ...
        "tax": "증세",
        "debt": "국채",
        "welfare_merge": "기존 복지 통합",
        "voucher": "청년 주거 바우처 포함",
        "get_ai": "이 설정으로 AI 스타일 해석받기",
        "rerun_ai": "재분석",
        "analysis_fail": "분석을 불러오지 못했습니다 다시 시도해주세요",
    ...
    "en": {
        ...
        "welfare_merge": "Welfare integration",
        "voucher": "Include youth housing voucher",
        "get_ai": "Get AI-style interpretation",
        "rerun_ai": "Re-analyse",
        "analysis_fail": "Could not load the analysis Please try again",
    ...
    "en": [
        {
            ...
            "body": "This case provided unconditional cash support to unemployed participants Employment effects were limited, but welfare discussions often focus on improved psychological security and life satisfaction",
        ...
        },
        {
            ...
            "body": "Canadian pilot programs and policy debates offer a useful lens for analysing poverty reduction, income security, and labour market participation",
...
# HELPERS
# -----------------------------
def card(label, value, note, color="#4F8CFF"):
    st.markdown(
        f"""
        <div class="card">
...
def insight_stream(text):
    ...
    for ch in text:
        shown += ch
        box.markdown(f'<div class="insight-box">{shown}▌</div>', unsafe_allow_html=True)
        time.sleep(0.005)
    box.markdown(f'<div class="insight-box">{shown}</div>', unsafe_allow_html=True)

def get_col(name_ko, name_en):
    return name_ko if LANG == "ko" else name_en

def funding_label_to_code(label):
...
def scenario_calculation(amount, target_label, funding_label, voucher_on):
    ...
    funding_code = funding_label_to_code(funding_label)
    ...
    annual_cost_adjusted = annual_cost * funding_cost_adjust

    base_unemp = 5.9
    amount_effect = amount * 0.014
    voucher_effect = 0.18 if voucher_on else 0
    target_effect = 0.12 if target_is_all else 0.24
    funding_penalty = {"tax": 0.08, "debt": 0.16, "merge": 0.11}[funding_code]
    unemp_change = -(amount_effect + voucher_effect + target_effect) + funding_penalty
    projected_unemp = max(3.8, base_unemp + unemp_change)

    base_housing = 27.2
    housing_drop = amount * 0.045 + (3.2 if voucher_on else 0)
    housing_burden = max(15.0, base_housing - housing_drop)

    gdp_effect = amount * (0.008 if target_is_all else 0.0045)
    gdp_effect *= {"tax": 0.82, "debt": 1.04, "merge": 0.72}[funding_code]
    gdp_effect = min(gdp_effect, 1.65)

    satisfaction = min(94, 45 + amount * 0.32 + (9 if voucher_on else 0) + (4 if not target_is_all else 0))
    fiscal_stability = max(18, 88 - annual_cost_adjusted * 0.55 - (10 if funding_code == "debt" else 0))
    feasibility = max(22, 82 - amount * 0.32 - (12 if target_is_all else 0) + (8 if funding_code == "merge" else 0))

    return {
        "annual_cost": annual_cost_adjusted,
...
def generate_simulation_insight(amount, target, funding, voucher, result):
    if LANG == "ko":
        ...
        return (
            f"현재 시나리오는 월 {amount}만원 기본소득, 지급 대상은 {target}, 재원 조달은 {funding}, "
            f"{voucher_txt} 설계입니다<br><br>"
            f"모델상 청년실업률은 현재 대비 약 {result['unemp_change']:.2f}%p 변화하고, "
            f"청년 주거부담률은 약 {result['housing_burden']:.1f}% 수준으로 완화되는 것으로 나타납니다 "
            f"다만 연간 재정 소요가 약 {result['annual_cost']:.1f}조원으로 추정되기 때문에 정책 효과보다 재원 설계가 더 중요한 변수입니다<br><br>"
            f"{risk}<br><br>"
            f"따라서 이 조건은 단순 현금 지급만으로 보기보다 청년 주거 안정, 고용서비스, 직업훈련, 지역 소비 정책을 묶은 패키지로 설계할 때 설득력이 커집니다"
        )
    else:
        ...
        return (
            f"This scenario assumes a monthly basic income of KRW {amount}0,000, targeted at {target}, funded through {funding}, "
            f"{voucher_txt}<br><br>"
            ...
            f"{risk}<br><br>"
            f"Overall, this scenario becomes more persuasive when basic income is combined with housing support, employment services, job training, and local consumption policy"
        )
    ...
    if LANG == "ko":
        if kind == "welfare":
            return (
                ...
                "기본소득은 청년에게 최소한의 선택권과 시간 여유를 제공할 수 있지만, 주거비 상승이 빠른 상황에서는 현금 지원만으로 효과가 흡수될 가능성도 있습니다<br><br>"
                "따라서 기본소득은 청년 공공임대, 주거 바우처, 고용 상담, 직업훈련과 연결될 때 복지 효과가 더 분명해집니다"
            )
        if kind == "business":
            return (
                "경영경제 관점에서 기본소득은 단기적으로 소비를 늘릴 수 있지만, 재원 조달 방식에 따라 경제적 파급효과가 달라집니다<br><br>"
                "증세는 재정 안정성은 높지만 소비 위축 논쟁이 생길 수 있고, 국채는 단기 수요 진작에는 유리하지만 장기 부담이 커질 수 있습니다<br><br>"
                "기업 입장에서는 청년층의 소비 여력이 커지는 업종과 세금 부담이 커지는 업종이 다르게 나타나기 때문에 산업별 영향 분석이 필요합니다"
            )
        return (
            "통합 정책 제언의 핵심은 전 국민 기본소득을 바로 도입하기보다 청년층을 대상으로 한 제한적 실험부터 시작하는 것입니다<br><br>"
        ...
        if kind == "business":
            return (
                ...
                "For businesses, the effects will differ by industry, because some sectors benefit from youth consumption while others face higher tax or cost pressure"
            )
        return (
...
st.sidebar.info(
    "한국어 기본 / English toggle" if LANG == "ko"
    else "Korean-first dashboard / English toggle included"
)
...
with tab1:
    ...
    row = main_df[main_df["year"] == selected_year].iloc[0]

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        card(T["youth_unemp"], f"{row['youth_unemployment']:.1f}%", "15–29 youth indicator" if LANG == "en" else "15~29세 청년 지표", "#4F8CFF")
    with c2:
        card(T["housing_burden"], f"{row['housing_burden']:.1f}%", "Housing cost / income" if LANG == "en" else "주거비 / 소득 기준", "#FFB020")
    with c3:
        card(T["ubi_cases"], f"{int(row['ubi_case_count'])}", "Selected global cases" if LANG == "en" else "주요 실험·논의 사례", "#32D583")
    with c4:
        card(T["welfare_spend"], f"{row['welfare_spending']:.1f}%", "Public social spending / GDP" if LANG == "en" else "GDP 대비 공공사회복지지출", "#A78BFA")

    section("핵심 구조" if LANG == "ko" else "Dashboard logic")
    ...
    with col_a:
        st.markdown(
            ...
            <b>① Problem</b><br><br>
            청년 고용 불안과 주거비 부담이 동시에 커지는 구조를 확인합니다
            </div>
            """ if LANG == "ko" else """
            <div class="policy-card">
            <b>① Problem</b><br><br>
            The dashboard first identifies the combined pressure of youth unemployment and housing costs
            </div>
            """,
            unsafe_allow_html=True
        )
    with col_b:
        st.markdown(
            ...
            <b>② Policy Tool</b><br><br>
            기본소득을 복지정책이자 소비·재정 정책으로 동시에 분석합니다
            </div>
            """ if LANG == "ko" else """
            <div class="policy-card" style="border-left-color:#FFB020;">
            <b>② Policy Tool</b><br><br>
            Basic income is analysed as both welfare policy and economic policy
            </div>
            """,
            unsafe_allow_html=True
        )
    with col_c:
        st.markdown(
            ...
            지급액, 대상, 재원, 주거 바우처 여부를 바꿔 정책 효과를 비교합니다
            </div>
            """ if LANG == "ko" else """
            <div class="policy-card" style="border-left-color:#32D583;">
            <b>③ Simulation</b><br><br>
            Users compare policy effects by changing payment amount, target group, funding method, and housing voucher design
            </div>
            """,
            unsafe_allow_html=True
        )
...
with tab2:
    ...
    fig = px.line(
        ...
        labels={
            "year": "연도" if LANG == "ko" else "Year",
            "youth_unemployment": "청년실업률 (%)" if LANG == "ko" else "Youth unemployment rate (%)"
        }
    )
    fig.update_traces(line=dict(width=4, color="#4F8CFF"), marker=dict(size=9))
    fig.update_layout(height=380, paper_bgcolor="#0F1117", plot_bgcolor="#0F1117")
    ...
    with col1:
        ...
        fig2 = px.bar(
            ...
            template="plotly_dark",
            labels={
                country_col: "국가" if LANG == "ko" else "Country",
                "youth_unemployment": "청년실업률 (%)" if LANG == "ko" else "Youth unemployment rate (%)"
            }
        )
        fig2.update_traces(marker_color=["#4F8CFF", "#32D583", "#A78BFA", "#FFB020", "#F97066", "#94A3B8"])
        fig2.update_layout(height=360, paper_bgcolor="#0F1117", plot_bgcolor="#0F1117")
    ...
    with col2:
        ...
        fig3 = px.area(
            ...
            labels={
                "year": "연도" if LANG == "ko" else "Year",
                "housing_burden": "주거부담률 (%)" if LANG == "ko" else "Housing burden rate (%)"
            }
        )
        fig3.update_traces(line=dict(width=3, color="#FFB020"), fillcolor="rgba(255,176,32,0.24)")
        fig3.update_layout(height=360, paper_bgcolor="#0F1117", plot_bgcolor="#0F1117")
    ...
    for i, case in enumerate(case_cards[LANG]):
        with case_cols[i]:
            st.markdown(
                f"""
                <div class="card">
                    <div style="color:#32D583; font-weight:750; margin-bottom:8px;">{case["tag"]}</div>
                    <div style="font-size:1.08rem; font-weight:800; margin-bottom:10px;">{case["title"]}</div>
                    <div style="color:#CBD5E1; line-height:1.55; font-size:0.9rem;">{case["body"]}</div>
                </div>
                """,
                unsafe_allow_html=True
            )
...
with tab3:
    ...
    with col1:
        ...
        fig4 = px.bar(
            ...
            template="plotly_dark",
            labels={
                "value": "부담 지수" if LANG == "ko" else "Burden index",
                "variable": "항목" if LANG == "ko" else "Metric",
                method_col: "재원 조달 방식" if LANG == "ko" else "Funding method"
            }
        )
        fig4.update_layout(height=390, paper_bgcolor="#0F1117", plot_bgcolor="#0F1117")
    ...
    with col2:
        ...
        fig5.add_trace(go.Scatter(
            ...
            y=consumption_df["low"],
            mode="lines",
            name="Low" if LANG == "en" else "낮은 지급액",
            line=dict(width=3)
        ))
        fig5.add_trace(go.Scatter(
            x=consumption_df["year"],
            y=consumption_df["medium"],
            mode="lines",
            name="Medium" if LANG == "en" else "중간 지급액",
            line=dict(width=3)
        ))
        fig5.add_trace(go.Scatter(
            x=consumption_df["year"],
            y=consumption_df["high"],
            mode="lines",
            name="High" if LANG == "en" else "높은 지급액",
            line=dict(width=3)
        ))
        fig5.update_layout(
            template="plotly_dark",
            ...
            plot_bgcolor="#0F1117",
            yaxis_title="GDP 대비 %" if LANG == "ko" else "% of GDP",
            xaxis_title="연도" if LANG == "ko" else "Year"
        )
    ...
    fig6.update_layout(height=430, paper_bgcolor="#0F1117", plot_bgcolor="#0F1117")
...
with tab4:
    ...
    with input_col:
        ...
        amount = st.slider(
            ...
            value=50,
            step=5,
            format="%d만원" if LANG == "ko" else "%d"
        )

        target_options = [T["youth_only"], T["all_people"]]
        target = st.radio(T["target"], target_options, index=0)

        funding_options = [T["tax"], T["debt"], T["welfare_merge"]]
        funding = st.radio(T["funding"], funding_options, index=0)
        ...
        st.markdown("---")
        st.metric(
            "연간 재정 소요 추정" if LANG == "ko" else "Estimated annual fiscal cost",
            f"{result['annual_cost']:.1f}조원" if LANG == "ko" else f"KRW {result['annual_cost']:.1f} tn"
        )
        st.metric(
            "소비 진작 효과" if LANG == "ko" else "Consumption stimulus",
            f"GDP 대비 {result['gdp_effect']:.2f}%" if LANG == "ko" else f"{result['gdp_effect']:.2f}% of GDP"
        )
    ...
    with output_col:
        ...
        fig7.add_trace(go.Scatter(
            ...
            mode="lines+markers",
            name=T["youth_unemp"],
            line=dict(width=4, color="#4F8CFF")
        ))
        fig7.add_hline(y=5.9, line_dash="dash", line_color="#94A3B8")
        fig7.update_layout(
            template="plotly_dark",
            height=330,
            title="청년 실업률 변화 예측" if LANG == "ko" else "Projected youth unemployment change",
            yaxis_title="%" ,
            xaxis_title="연도" if LANG == "ko" else "Year",
            paper_bgcolor="#0F1117",
            plot_bgcolor="#0F1117"
        ...
        with col_a:
            ...
            fig8.add_trace(go.Bar(
                x=["현재", "시뮬레이션"] if LANG == "ko" else ["Current", "Simulation"],
                y=housing_path,
                marker_color=["#94A3B8", "#FFB020"]
            ))
            fig8.update_layout(
                template="plotly_dark",
                height=320,
                title="청년 주거 부담률 변화" if LANG == "ko" else "Youth housing burden change",
                yaxis_title="%",
                paper_bgcolor="#0F1117",
                plot_bgcolor="#0F1117"
            )
        ...
        with col_b:
            fig9 = go.Figure(go.Indicator(
                ...
                number={"suffix": "조원" if LANG == "ko" else " tn"},
                gauge={
                    "axis": {"range": [0, 360]},
                    "bar": {"color": "#A78BFA"},
                    "steps": [
                        {"range": [0, 80], "color": "rgba(50,213,131,0.22)"},
                        {"range": [80, 180], "color": "rgba(255,176,32,0.22)"},
                        {"range": [180, 360], "color": "rgba(249,112,102,0.22)"},
                    ],
                },
                title={"text": "연간 재정 소요" if LANG == "ko" else "Annual fiscal cost"}
            ))
            fig9.update_layout(
                template="plotly_dark",
                height=320,
        ...
        fig10.add_trace(go.Scatter(
            ...
            mode="lines",
            name="GDP effect",
            line=dict(width=4, color="#32D583"),
            fillcolor="rgba(50,213,131,0.25)"
        ))
        fig10.update_layout(
            template="plotly_dark",
            height=300,
            title="소비 진작 효과 GDP 대비 %" if LANG == "ko" else "Consumption stimulus as % of GDP",
            yaxis_title="%",
            xaxis_title="연도" if LANG == "ko" else "Year",
            paper_bgcolor="#0F1117",
            plot_bgcolor="#0F1117"
        ...
        radar_categories = (
            ["소득 안정", "주거 안정", "고용 완화", "재정 안정", "정책 실행성"]
            if LANG == "ko"
            else ["Income security", "Housing stability", "Employment relief", "Fiscal stability", "Feasibility"]
        )
        radar_values = [
            result["satisfaction"],
        ...
        radar_values = [max(0, min(100, v)) for v in radar_values]
    ...
    if st.button(T["get_ai"], key="ai_simulation"):
        with st.spinner("분석 중..." if LANG == "ko" else "Analysing..."):
            time.sleep(0.5)
        insight_stream(generate_simulation_insight(amount, target, funding, voucher, result))

# -----------------------------
# TAB 5 POLICY RECOMMENDATIONS
# -----------------------------
with tab5:
    result = st.session_state.get("last_result", scenario_calculation(50, T["youth_only"], T["tax"], True))
    ...
    s1.metric(T["monthly_amount"], f"{amount}만원" if LANG == "ko" else f"KRW {amount}0,000")
